...
public class SwiftHyperpayPlugin: NSObject, FlutterPlugin, SFSafariViewControllerDelegate, UIAdaptivePresentationControllerDelegate, PKPaymentAuthorizationViewControllerDelegate {
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        if(call.method == "setup_service") {
            ...
            case Brand.UNKNOWN:
                result(
                    FlutterError(
                        code: "0.1",
                        message: "الرجاء تقديم علامة تجارية صالحة",
                        details: ""
                    )
                )
        ...
        } else if (call.method == "start_saved_card_transaction") {
    ...
    private func onSavedCard(args:Dictionary<String, Any>) {
    ...
    private func onApplePay(args:Dictionary<String, Any>) {
        ...
        if Bundle.main.bundleIdentifier! == "com.abyan.capital" {
        ...
        paymentRequest.paymentSummaryItems = [
            PKPaymentSummaryItem(label: "Abyan",
    ...
    private func checkCreditCardValid(result: @escaping FlutterResult) {
        if !OPPCardPaymentParams.isNumberValid(self.cardNumber, luhnCheck: true) {
            result(
                FlutterError(
                    code: "1.1",
                    message: "معلومات البطاقة غير صحيحة \(self.brand)",
                    details: ""
                )
            )
        }
            ...
            result(
                FlutterError(
                    code: "1.3",
                    message: "شهر انتهاء الصلاحية غير صالح",
                    details: "يجب أن يكون الشهر بتنسيق MM"
                )
            )
        }
            ...
            result(
                FlutterError(
                    code: "1.4",
                    message: "سنة انتهاء الصلاحية غير صالحة",
                    details: ""
                )
            )
        }
